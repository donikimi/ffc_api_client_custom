...
pkg.env$FAIL_YEARS_DATA <- 1  # how many years of data do we require to proceed? assigned to FFCProcessor - default here so we can lower it during testing # edit by DhKim = 1.
...
FFCProcessor <- R6::R6Class("FFCProcessor", list(
  ...
  warn_years_data = 5,  # we'll warn people if we have this many years or fewer after filtering
  ...
  set_up = function(gage_id, timeseries, comid, token){
    ...
    if(!is.na(gage_id)){
	  ...
	  self$gage$gage_fail_years_data <- self$fail_years_data # edit by DhKim: pass down fail_years_data to USGSGage$get_data()
    ...
    # Filter Timeseries! # comment by DhKim: this could be turned off but not now.
